...
//				 return wc_price( wc_get_customer_total_spent( $user->ID ) );
			...
			case 'last_order' :
				...
				if ( ! empty( $orders ) ) {
					$order = $orders[0];
					return '<a href="' . admin_url( 'post.php?post=' . $order->get_id() . '&action=edit' ) . '">' . _x( '#', 'hash before order number', 'woocommerce' ) . $order->get_order_number() . '</a> &ndash; ' . date_i18n( get_option( 'date_format' ), strtotime( $order->get_date_created() ) );
				} else {
					return '-';
				}
	...
	public function get_columns() {
		$columns = array(
			...
			'wallet'           => __( 'Wallet Money', 'woocommerce' ),
...
$AssignProductListTable = new WC_Report_Customer_List();
printf( '<div class="wrap" id="product-list-table"><h2>%s</h2>', __( 'Customer Wallet Details', 'your-textdomain' ) );
...
$AssignProductListTable->prepare_items(); // this will prepare the items AND process the bulk actions
$AssignProductListTable->display();
